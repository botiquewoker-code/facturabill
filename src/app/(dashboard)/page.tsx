...
const HOME_VISIBILITY_KEY = "facturabill-home-visibility";
...
type HomeVisibility = Record<HomeVisibilityKey, boolean>;
...
function readHomeVisibility(): HomeVisibility {
  if (typeof window === "undefined") {
    return DEFAULT_HOME_VISIBILITY;
  }

  try {
    const raw = window.localStorage.getItem(HOME_VISIBILITY_KEY);
...
export default function DashboardHome() {
  ...
  const [draftHomeVisibility, setDraftHomeVisibility] =
    useState<HomeVisibility>(DEFAULT_HOME_VISIBILITY);
  const [hasLoadedHomeVisibility, setHasLoadedHomeVisibility] = useState(false);
  ...
  const hasHiddenSections = Object.values(homeVisibility).some((value) => !value);
  ...
  useEffect(() => {
    const hydrateSearchData = () => {
      const savedHomeVisibility = readHomeVisibility();
      ...
      setHasLoadedHomeVisibility(true);
    };
  ...
  useEffect(() => {
    if (typeof window === "undefined" || !hasLoadedHomeVisibility) {
      return;
    }

    window.localStorage.setItem(
      HOME_VISIBILITY_KEY,
      JSON.stringify(homeVisibility),
    );
  }, [hasLoadedHomeVisibility, homeVisibility]);
  ...
  const hasBusinessActivity =
    clientes.length > 0 || historial.length > 0 || borradores.length > 0;
  const heroTitle =
    language === "es"
      ? hasBusinessActivity
  ...
  function saveHomeVisibilityPreferences() {
    ...
    if (typeof window !== "undefined") {
      window.localStorage.setItem(
        HOME_VISIBILITY_KEY,
        JSON.stringify(draftHomeVisibility),
      );
    }
